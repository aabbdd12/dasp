...
if  `tjob1'==1 {;
tempname mat10;
qui mean elas_m elas_l elas_u , over(decile);
tempname rtp ;
matrix `rtp' = r(table)' ;
...
egen  double `totalexp_sum'   =  total(hhwgt*`1'), by(`decile')
egen  double `dec_s_w'        =  total(hhwgt)    , by(`decile')
...
if   `tjob6'==6 |  `gjob4'==4  {
qui nlcom   ( _b[`vv13']/ _b[`vv00'] ) + ( _b[`vv11'] ) / ( _b[`vv2'] * _b[`vv3']) + ( _b[`vv12'] ) / ( _b[`vv2'] * _b[`vv4']) , iterate(40000) 
local est=el(r(b),1,1)*100
local se=el(r(V),1,1)^0.5*100
local lb=(el(r(b),1,1)-1.96*el(r(V),1,1)^0.5)*100
local ub=(el(r(b),1,1)+1.96*el(r(V),1,1)^0.5)*100
