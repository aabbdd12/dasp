...
capture program drop prjobstat;
program define prjobstat, eclass;
version 9.2;
...
qui replace `Variable' = "Population" in `indig1';
...
if "`stat'" == "exp_pcs" | "`stat'" == "exp_pcs2" 	local rnam `"`rnam' "P_Value""' ;
};
...
local tmp mcstr("``j''",1,30);
local cnam `"`cnam' "`tmp' `uni`i''""';
};
};

local cnam `"`cnam' "All items""';



cap matrix rownames `zz' = `rnam' ;
cap matrix colnames `zz' = `cnam' ;
