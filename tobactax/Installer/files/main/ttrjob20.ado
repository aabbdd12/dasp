...
tempname pcexp pcexpcig;
qui gen `pcexp' = `1'/`hsize' ; 
qui gen `pcexpcig' = `2'/`hsize' ; 
qui sum `pcexp' [aw=`_fw'];
return scalar est4   = `r(mean)';

qui sum `pcexpcig' [aw=`_fw'];
...
capture program drop ttrjob20;
program define ttrjob20, eclass;
version 9.2;
...
if ("`lan'"~="fr")  local cnam `"`cnam' "Per capita Expenditures""';
if ("`lan'"~="fr")  local cnam `"`cnam' "Per capita Exp on Tobacco""';
