...
if ("`type'"!="nor" | `al'==0)    qui nlcom (_b[`num']/_b[`hs'] + _b[`snum']/_b[`hsr']),        iterate(50000) ;
if ("`type'"=="nor" &  `al'!=0)   qui nlcom (_b[`num']/_b[`hs']+ _b[`snum']/_b[`hsr'])/((_b[`hsy']/_b[`hsr'])^`al'),  iterate(50000);
cap drop matrix _vv;
matrix _vv=r(V);
local std = el(_vv,1,1)^0.5;
};
