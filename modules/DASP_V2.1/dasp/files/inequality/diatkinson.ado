...
#delim ;
set more off;
cap program drop diatkinson2;  
program define diatkinson2, rclass;    
version 9.2;         
syntax varlist (min=1 max=1) [, HSize(varname) HWeight(varname) epsilon(real 1)  CI(real 5)  CONF(string) LEVEL(real 95) VAB(real 0)];
tokenize `varlist';
...
capture program drop diatkinson;
program define diatkinson, eclass;
version 9.2;
...
diatkinson2 `1' , hweight(`hweight1') hsize(`ths1')  epsilon(`epsilon') conf(`conf') level(`level') vab(`vab');
if (`vab'==1) {;
tempvar va vb vc;
qui gen `va'=__va;
qui gen `vb'=__vb;
...
diatkinson2 `2' , hweight(`hweight2') hsize(`ths2') epsilon(`epsilon')  conf(`conf') level(`level') vab(`vab') ;
if (`vab'==1) {;
tempvar vd ve vf;
qui gen `vd'=__va;
qui gen `ve'=__vb;
...
matrix gra=
(
((1/(`epsilon'*(`epsilon'-1)))*($ws2^(`epsilon'-1)))/($ws3^`epsilon')\
(1/`epsilon')*($ws1 )*($ws2^(`epsilon'-2))/($ws3^`epsilon')\
(1/(1-`epsilon'))*(($ws1 )*($ws2^(`epsilon'-1)))/($ws3^(`epsilon'+1))\
-((1/(`epsilon'*(`epsilon'-1)))*($ws5^(`epsilon'-1)))/($ws6^`epsilon')\
-(1/`epsilon')*($ws4 )*($ws5^(`epsilon'-2))/($ws6^`epsilon')\
-(1/(1-`epsilon'))*(($ws4 )*($ws5^(`epsilon'-1)))/($ws6^(`epsilon'+1))
);
};
...
local tyind1="ATK._D1";
local tyind2="ATK._D2";
