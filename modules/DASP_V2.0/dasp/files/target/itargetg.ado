...
DEC(int 9)
DSTD(int 1)
type(string)  LRES(int 0) SRES(string) DCI(string) LEVEL(real 95) CONF(string)
...
if ("`hgroup'"~="") {;
	...
	di _n as text "{col 4} targetging polulation groups and poverty";
       			    di as text     "{col 5}targetging  groups  :  `targetg'";
                            di as text     "{col 5}targetging  scheme  :  `scheme'";
                            di as text     "{col 5}Normalised by cost :  `cnor'";
	...
	.`table'.titles "       " "  Share    " "         "   "Population"  "Group    " ;
	.`table'.sep, mid;
	local nalt = "ddd";
	forvalues i=1/`k2'{;
