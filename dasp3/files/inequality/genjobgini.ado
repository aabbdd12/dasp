...
mbasicgini `varlist' ,  hsize(`_ths')  hgroup(`hgroup_`i'')  xrnames(`xrnames') rank(`rank') type(`type')
matrix _ms_`i' = e(mmss)'
local tmatna `tmatna' _ms_`i'
}
}

mbasicgini `varlist' ,  hsize(`_ths')    xrnames(`xrnames') rank(`rank')  type(`type')
#delimit cr
...
matrix rownames _ms_pop = `varlist'
	...
	if ("`rank'"=="" | "`rank'"=="`1'" ) & ("`type'"=="")     local index = "Gini";
	if ("`rank'"~="" & "`rank'"~="`1'" ) & ("`type'"=="")     local index = "Concentration";
	if ("`rank'"=="") & ("`type'"=="abs")  local index = "Absolute Gini";
	if ("`rank'"~="") & ("`type'"=="abs")  local index = "Absolute concentration";
...
if ("`hgroup'" ~= "" ) local gro = 1
	if "`xtit'"==""   local xtit Table ##: " `index' index"
