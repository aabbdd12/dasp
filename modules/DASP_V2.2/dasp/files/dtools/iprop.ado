...
cap program drop iprop2;  
program define iprop2, rclass ;    
version 9.2;         
syntax varlist (min=1 max=1) [, HSize(varname) HWeight(varname) GNumber(int -1) CI(real 5)  CONF(string) LEVEL(real 95)];
preserve;
tokenize `varlist';
                     qui drop if `1'>=. ;
...
tempvar _in;
 qui gen    `_in' = (`1' == `gnumber');
 qui replace `hs' = `hs' * `_in';
...
capture program drop iprop;
program define iprop, eclass;
version 9.2;
syntax varlist(min=1)[, HSize(varname)   CONF(string)
...
local kk = gn1[`k'];
local label`k'  : label (`1') `kk';
if "`label`k''"=="" local label`k'="Group: `kk'";
qui replace `Variable' = "`k': `label`k''" in `k';

iprop2 `1' ,  hweight(`hweight')  hsize(`_ths')   conf(`conf') 
level(`level')  gnumber(`kk');
...
iprop2 `1' ,   hweight(`hweight') hsize(`_ths')   conf(`conf') level(`level');
