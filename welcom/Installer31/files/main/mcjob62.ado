...
#delimit;
capture program drop mcjob62;
program define mcjob62, eclass;
version 9.2;
syntax varlist(min=1)[, HSize(varname) HGroup(varname) LAN(string) XRNAMES(string)  AGGRegate(string) PCEXP(varname)  WAPPR(int 1) MODEL(int 1) GVIMP(int 0) SUBS(real 0.4) MOVE(int 1) EPSILON(real 0.5) STEP(int 1)];
...
mcjobatkinson `tot_imp',   hs(`hsize') aehs(`aehs')  lan(`lan')   xrnames(`slist')  pcexp(`pcexp')    move(`move')  epsilon(`epsilon') ;
tempname mat62tot ;
matrix `mat62tot'= e(est); 
};
...
mcjobatkinson `flist',   hs(`hsize') aehs(`aehs')  lan(`lan')   xrnames(`slist')  pcexp(`pcexp')    move(`move')  epsilon(`epsilon') ;
cap drop `drlist';
tempname mat62 ;
matrix `mat62'= e(est);

if (`wappr'!=1) {;
local rowsize = rowsof(`mat62');
local colsize = colsof(`mat62');
forvalues i=1/`rowsize' {;
 matrix `mat62'[ `i',`colsize'] = el(`mat62tot',`i',1);
};
