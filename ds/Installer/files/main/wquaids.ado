...
"must specify {cmd:expenditure()} or {cmd:lnexpenditure()}"
	...
	Display, level(`level') snames(`snames')
...
program Display
	...
	syntax , [Level(cilevel) SNames(string)] 
...
title( " - Expenditure elasticities ");
...
matlist B_NS_e, border(all)  format(%10.4f)  twidth(14) left(2) 
title( " - Price elasticities ");
#delimit cr
end
//exit
