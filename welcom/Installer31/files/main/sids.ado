...
forvalues i=1/`r(N)' {;
local tem=value[`i'];
local grlab`tem' = word(label[`i'],1);
};
};
};
...
local gr = gn1[`g'];
local lvgroup:value label `hgroup';
cap drop LnPrice_`grlab`gr'';
qui gen LnPrice_`grlab`gr'' =  LnPrice*(`gr'==`hgroup');
local price_group `price_group'  LnPrice_`grlab`gr'';
};
...
if `incint' == 1 {;
cap drop LnHH_Inc_`grlab`gr'';
qui gen LnHH_Inc_`grlab`gr'' =  LnHH_Inc*(`gr'==`hgroup');
if (`inc'!=0) local subcom `subcom'  LnHH_Inc_`grlab`gr'';
};

forvalues i=1/`nigr' {;
cap drop LnPrice_`grlab`gr'';
if `i' <=  9  gen LnPrice_`iname'_0`i'_`grlab`gr'' =  LnPrice*(`i'==`incpar')*(`gr'==`hgroup');
if `i' >= 10  gen LnPrice_`iname'_`i'_`grlab`gr''  =  LnPrice*(`i'==`incpar')*(`gr'==`hgroup');
if `i' <=  9   local price_group `price_group'  LnPrice_`iname'_0`i'_`grlab`gr'';
if `i' >= 10   local price_group `price_group'  LnPrice_`iname'_`i'_`grlab`gr'';
};
};
};
