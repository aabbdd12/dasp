...
capture program drop cdf2;
program define cdf2, rclass;
version 9.2;
...
qui replace `fcdf'=`r(mean)' in `j';
};
qui keep in 1/101;
set matsize 101;
cap matrix drop _xx;
mkmat `fcdf', matrix (_xx);
restore;
end;

capture program drop cdf;
program define cdf, rclass;
version 9.2;
...
local ftitle = "`tit0'"+"`tit3'"+" `tit1' Curve`tits' ";
                   local ytitle = "`tit4'`tit2'";
...
if ("`hgroup'"=="") {;
local label`f'  =  "``k''";
if ("`dif'"=="c1") local label`f'  = "CDF_``k'' - CDF_`1'";
cdf2 `fw' ``k'' `alpha' `type' `min' `max';
};
...
cdf2 `fw' `1' `alpha' `type'  `min' `max' `hgroup' `k';
};
svmat float _xx;
cap matrix drop _xx;
rename _xx1 _cory`k';
