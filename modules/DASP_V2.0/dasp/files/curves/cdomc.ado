...
version 9.2;
syntax varlist(min=1)[,  HWEIGHT(string) HSize(string) COMPonent(string) ALpha(real 1) PLINE(real 10000)
cnor(string)
];
preserve;
tokenize `varlist';
...
if (`al1' ~= 0 & `pline' ~=0 )           qui replace     `numi' = `alpha'/`pline'*`hs'*((`pline'-`1')/`pline')^`al1'  if (`pline'>`1');
                                         qui replace    `numi'  = `alpha'*`num'+`pline'*`numi';
};
};
...
version 9.2;
syntax varlist(min=1)[,  HWEIGHT(string) HSize(string) COMPonent(string) ALpha(real 1) 
...
HSize(varname)  clist(varlist)  ORDER(real 1) PLINE(real 10000)
cnor(string)
DEC(int 9)
DSTD(int 1)
...
hsize(`hsize') component(``k'')   alpha(`alpha') pline(`pline') cnor(`cnor');
...
if ("`clist'"~="") {;
	...
	.`table'.numfmt %16.0g  %16.`dec'f  %16.`dec'f %16.`dec'f;
                            di as text     "{col 4}Normalised by cost :  `cnor'";
	...
	.`table'.sep, top;
	.`table'.titles "Component(s)  " "Population  " "FGT index"  "CD: Impact on "   ;
	...
	forvalues i=1/`k2'{;
		  ...
		  if (`dstd'==1 | (`i'/2!=round(`i'/2))) .`table'.row `Variable'[`i'] `EST'[`i']  `EST2'[`i'] `EST3'[`i'];
...
local kk2 = 2;
  tempname table;
	...
	di _n as text "{col 1} targetging polulation groups and poverty";
         
	.`table'.sep, top;
	.`table'.titles  "FGT index"  " CD: Impact on "   ;
...
gcdomcc2 `inc', hweight(`hweight') component(`inc')
hsize(`hsize')  alpha(`alpha') min(`min') max(`max')
cnor(`cnor') dci(`dci') level(`level') conf(`conf') label(population);
