...
capture program drop mbasicstati;
program define mbasicstati, eclass;
syntax varlist(min=1) [, HSize(varname)  HGROUP(varname) AEHS(varname)   XRNAMES(string)  STAT(string) MTRANS(int 1) CHANGE(string) DIF1(int 0)];
preserve; 

tokenize `varlist';
_nargs   `varlist';
tempvar we ga0 ga00 ga01 ga10 ga1 hy;
...
qui gen `pcexp' = `1' ; 
  ...
  qui replace `ga0' = `hsize'*`pcexp';
  qui replace `ga00' = `pcexp';
...
qui sum `ga00'   [aweight= `we'*`hsize']  if `hgroup' == `x';
matrix `aa'[1,`i'] = r(mean);
if ("`stat'"=="total") {;
matrix `aa'[1,`i'] = r(sum);
};
...
tempvar pcexp`s' ;
qui gen `pcexp`s'' = ``s'' ;
qui replace `ga01' = `pcexp`s'' ;
qui replace `ga1'  = `hsize'*`pcexp`s'' ;
...
qui sum `ga01' [aweight= `we'*`hsize']  if `hgroup' == `x';;
matrix `aa'[`sp1',`i'] = r(mean);
if ("`stat'"=="total" ) {;
...
forvalues s=2/$indica {;
if ("`stat'"=="mean" ) qui svy: ratio `ga10_`s'' / `hsize' , over(`hgroup');

if ("`stat'"=="total" ) qui svy: total `ga10_`s''           , over(`hgroup');
if ("`change'"=="rel" ) qui svy: ratio `ga10_`s''/`ga0'      , over(`hgroup');
local sp1 = `s' ;
forvalues i=1/`ngr' {;
...
capture program drop genjobstatigr;
program define genjobstatigr, eclass;
version 15;
...
//set trace on ;
set tracedepth 2;
mbasicstati `varlist' ,  hsize(__hs`j')    hgroup(`hgroup_`i'')  xrnames(`xrnames')  stat(`stat') mtrans(0) change(`change');
if `j' == 1 {;
...
mbasicstati `varlist' ,  hsize(__hs`j')   xrnames(`xrnames')  stat(`stat') mtrans(0) change(`change');
if `j' == 1 {;
