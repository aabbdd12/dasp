...
/* small change by A. Araar */

# delimit ;
cap program drop xml_taba;
program define xml_taba;
version 8;
...
foreach opt in Format Lines FONt LOng WIde STARs NOLabel SHOWeq save SHeet STATS
			   ...
			   Title NOADJust ADJust RNames CEq CNames NOTEs SAVEMat Constant CWidth SAVEPath {;
...
tempname M;
    if `rc'==0 {;
		...
		syntax [, Format(passthru) Lines(string asis) FONt(string asis)
				  ...
				  Title(string) RNames(passthru) CEq(passthru) CNames(passthru) NOTEs(string asis)
				];
...
if ~missing("`replace'") {;
		...
		file write `outfile' "<Description>created using xml_taba</Description></DocumentProperties>";
...
if ~missing("`cwidth'") {;
	...
	if ~missing("`title'") {;
			...
			local title : subinstr local title ">" "&gt;", all;
			local ltitle "<Row ss:Index='`addindex++''><Cell ss:StyleID='s`fno'_Title'  ss:MergeAcross='`rc''>
...
program define updateopts;
	...
	quietly findfile xml_taba.ado;
	local fname "`r(fn)'";
	local fname : subinstr local fname "xml_taba.ado" "xml_taba_options.txt";
