...
cap program drop icvar2;  
program define icvar2, rclass ;    
version 9.2;         
syntax varlist (min=1 max=1) [, HSize(varname) HWeight(varname) HGroup(varname)   GNumber(int -1) CI(real 5)  CONF(string) LEVEL(real 95)];
preserve;
tokenize `varlist';
                     qui drop if `1'>=. ;
...
local est  =  ($ws1*$ws2/$ws3^2-1)^0.5;
local temp = 0.5*($ws1*$ws2/$ws3^2-1)^-0.5;
cap drop matrix mat;
matrix mat=e(V);
cap drop matrix gra;
matrix gra=
...
capture program drop icvar;
program define icvar, eclass;
version 9.2;
syntax varlist(min=1)[, HSize(varname) HGroup(varname)  CONF(string)
...
icvar2 ``k'' , hweight(`hweight')  hsize(`_ths')    conf(`conf') level(`level');
...
icvar2 `1' ,  hweight(`hweight')  hsize(`_ths')   conf(`conf') 
...
icvar2 `1' ,   hweight(`hweight') hsize(`_ths')   conf(`conf') level(`level');
...
local 1kk = $indica;
local index = "Coffecient of variation";
