...
#delim ;
set more off;
cap program drop dicvar2;  
program define dicvar2, rclass;    
version 9.2;         
...
capture program drop dicvar;
program define dicvar, eclass;
version 9.2;
...
dicvar2 `1' , hweight(`hweight1') hsize(`ths1')  conf(`conf') level(`level') vab(`vab');
if (`vab'==1) {;
tempvar va vb vc;
qui gen `va'=__va;
qui gen `vb'=__vb;
...
dicvar2 `2' , hweight(`hweight2') hsize(`ths2')  conf(`conf') level(`level') vab(`vab') ;
if (`vab'==1) {;
tempvar vd ve vf;
qui gen `vd'=__va;
qui gen `ve'=__vb;
...
local temp1 = 0.5*($ws1*$ws2/$ws3^2-1)^-0.5;
local temp2 = 0.5*($ws4*$ws5/$ws6^2-1)^-0.5;
...
(
 `temp1'*$ws2/$ws3^2\
 `temp1'*$ws1/$ws3^2\
-`temp1'*2*$ws1*$ws2/$ws3^3\
-`temp2'*$ws5/$ws6^2\
-`temp2'*$ws4/$ws6^2\
 `temp2'*2*$ws4*$ws5/$ws6^3
);
...
local tyind1="CVAR_D1";
local tyind2="CVAR_D2";
