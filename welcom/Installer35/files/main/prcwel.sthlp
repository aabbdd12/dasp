...
{p 0 4}  {cmd: Information of indirect effect(s)} {p_end}

{p 0 4} {cmdab:ioc}   Add the option ioc(1) to compute the indirect effect(s). {p_end}

{p 0 4} {cmdab:allind}   Add the option allind(1) if the direct effects of price shocks are not indicated in the TAB Direct Effect(s).  {p_end}
...
;
{txt}      ({stata "welcom_examples ex_prc_05    ":example 5: click to run in command window})
{txt}      ({stata "welcom_examples ex_prc_db_05 ":example 5: click to run in dialog box})
