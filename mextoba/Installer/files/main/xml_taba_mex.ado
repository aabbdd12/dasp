*! version 3.35 17Oct2007 M. Lokshin, Z. Sajaia
/* small changes by A. Araar */

# delimit ;
cap program drop xml_taba_mex;
program define xml_taba_mex;
version 8;

syntax [anything] [, STYle(string) EXcelpath(passthru) CAlcpath(passthru) updateopts Noisily *];
...
foreach opt in Format Lines FONt LOng WIde STARs NOLabel SHOWeq save SHeet STATS
			   ...
			   Title NOADJust ADJust RNames CEq CNames TOPNOTEs NOTEs SAVEMat Constant CWidth SAVEPath {;
...
tempname M;
    if `rc'==0 {;
		...
		syntax [, Format(passthru) Lines(string asis) FONt(string asis)
				  ...
				  RBlanks(string asis) TBlanks(numlist max=1 >0 integer) CBlanks(numlist >=0) CWidth(string)
				  Title(string) RNames(passthru) CEq(passthru) CNames(passthru) TOPNOTEs(string asis) NOTEs(string asis)
				];
		...
		if ~_rc {;
			if rowsof(`first')!=rowsof(`first'_STARS) |
			   colsof(`first')!=colsof(`first'_STARS) {;
				display as error "matrices `first' and `first'_STARS must have same dimensions";
				exit 503;
			};
		...
		syntax  [, Format(passthru) Lines(string asis) FONt(string asis)
				...
				CBlanks(numlist >=0) CWidth(string)
				Title(string) CEq(passthru) CNames(passthru) TOPNOTEs(string asis) NOTEs(string asis) Constant(string) *
		];
...
if ~missing("`b'") | length("`sheet'")>31 {;
	display as error "not a valid Excel sheet name";
	exit 198;
};
...
if missing("`append'`replace'") {;
	capture confirm file "`save'";
	if ~_rc {;
		display as error "file `save' already exists";
		exit 602;
	};
	else {;
		local replace "replace";
...
drop _all;
quietly {;
   	savetodataset `M' `M'_STARS, `format' `sd2' `rnames' `cnames' `ceq' etitle(`"`etitle'"');

	local starnote "note: ";
	forvalues s = 1/`: list sizeof stars' {;
		local starnote "`starnote' `: word `s' of `stars'' - `: word `s' of `starchars'';";
	};

	capture confirm matrix `M'_STARS;
	if _rc local starnote;
		...
		while (~missing(`"`rblanks'"')) {;
			...
			if ~missing(`rm') {;
				...
				foreach var of varlist f_* {;
					replace `var' = cond(missing("`1'"),`var'[int(r(min)/10)+3], "0_" + trim("`1'")) in `N';
				};
...
if ~missing("`replace'") {;
        ...
        local xmlheader  "<?xml version='1.0' encoding='ISO-8859-1' standalone='yes'?>
        ...
        file read `ofile' line;  // Description, sheet names
        ...
        file write `outfile' `"`styles'"' _n;
		file read  `ofile'  line;                     // sheetnames
        local sheetID=0;
		local sheet  "`lsheet'";
...
if ~missing("`cwidth'") {;
	...
	if ~missing("`title'") {;
			...
			local title : subinstr local title ">" "&gt;", all;
			local ltitle "<Row ss:Index='`addindex++'' ss:AutoFitHeight='1' ss:Height='22'> ><Cell ss:StyleID='s`fno'_Title'  ss:MergeAcross='`rc'' ss:HRef="#table_11!A1">
	...
	local notes : subinstr local notes ">" "&gt;", all;
  	local --addindex;
	while (~missing(`"`topnotes'"')) {;
		_parse comma 1 topnotes : topnotes;
		gettoken numa txt : 1;
		capture confirm integer number `numa';
		if _rc {;
			local txt "`numa' `txt'";
			local numa=0;
		};
	  	local ++numa;
 		local addindex = `addindex'+`numa';
		file write  `outfile' `"<Row ss:Index='`addindex''><Cell ss:StyleID='s`fno'_Note'><Data ss:Type='String'>`txt'</Data></Cell></Row>"';
		local topnotes : subinstr local topnotes "," "";
	};
		local addindex=`numa'+`addindex' + 2;
	};
...
forvalues i = 1/`=_N' {;
	...
	foreach var of local vlist {;
		...
		local dtype = cond(upper(substr("`f'", 6, 1))=="N" & real(`"`datum'"')!=., "Number", "String");
		if (`mrg'>0) {;
...
file write  `outfile' "<Row ss:Index='`addindex++''><Cell ss:StyleID='s`fno'_Note'><Data ss:Type='String'>`starnote'</Data></Cell></Row>";
...
program define Mkemat, rclass;
	...
	tempname bbc bc bbs bs df;

	matrix `df' = J(`nnames', 1, .z);
	if ~missing("`stats'") {;
	...
	while ~missing("`first'") {;
		...
		nobreak {;
			...
			tempname B V VV SC tB tV tSTat;
	   		matrix `B'  = e(`eb');
	   		matrix `V'  = e(`eV');
			if ~missing("`noadj'") {;
			...
			matrix `df'[`ni',1] = e(df_r);
			if ~missing("`stats'") {;
				GetStats "`stats'" `bs';
				matrix          `bbs' = J(rowsof(`bs'), 1,.x),`bs', `bs', J(rowsof(`bs'), 1,.x);
		...
		forvalues i = 1/`=rowsof(`bc')' {;
			if ~missing("`noadj'") {;
				matrix `tSTat' = nullmat(`tSTat') \ (`tB'[1, `i']/sqrt(`tV'[`i', `i']), .z);;
			};
			else {;
				matrix `tSTat' = nullmat(`tSTat') \ (`bc'[`i', 1]/`bc'[`i', 2], .z);
			};
			if ~missing(`"`leform'"') {;
				matrix `bc'[`i', 1] = exp(`bc'[`i', 1]);
				matrix `bc'[`i', 2] = `bc'[`i', 1]*`bc'[`i', 2];
			};
		};

		matrix 			`bc' = `bc', `tSTat';
	 	matrix coleq    `bc' = `name'|`ni';
		matrix colnames `bc' = "`coef'" `cap' tstat mis;
	...
	matrix `bbc' = `bbc' \ nullmat(`tmpst');
   	...
   	if missing("`long'") {;
		...
		foreach col of local cequ {;
			...
			if ~_rc & `ni'>1 {;
				matrix `tsc' = `tsc', J(rowsof(`tsc'), (`ni'-1)*4, .z);
			};
			capture matrix `tst' = `tmp'["`rst':",....];
			if ~_rc & `ni'>1 {;
				local rownames : rowfullnames `tst';
				matrix `tst' = J(rowsof(`tst'), (`ni'-1)*4, .x), `tst';
	...
	local cnames_old : colnames `bbc', quoted;
	local cnames;
	forvalues j = 1/`rc' {;
		...
		local cnames `"`cnames' `: word `jC' of `cnames_old''"';
 		forvalues i = 1/`rr'{;
			...
			if mod(`j',`sdc'+1)==`sdc' & mod(`i', `sdr'+1)==`sdr' {;
				local d_tstat = `bbc'[`i',`jC'+`base'];
				local p       = 2*cond(`df'[(`j'+`sdc')/`base',1]==.,1-norm(abs(`d_tstat')),
																	 ttail(`df'[(`j'+`sdc')/`base',1],abs(`d_tstat')));
				forvalues s =1/`: list sizeof stars' {;
				...
				if ~missing("`tstat'") {;
					matrix `D'[`i',`j']=`d_tstat';
				};
				else {;
					if ~missing("`pvalue'") {;
						matrix `D'[`i',`j']=cond(missing(`p'), .z, `p');
					};
				};
			};
		};
...
cap program drop formateqnames;
...
program define savetodataset;
	...
	forvalues i = 1/`rr' {;
		...
		if ("`: roweq `tmp''"!="`roweq'" & "`: roweq `tmp''"!="_") {;
			set obs `++obs';
			replace d_0 = "`: roweq `tmp''" in `obs';
			replace id  = `i'*10 - 4 in `obs';
			local roweq "`: roweq `tmp''";
		};
	};
   	forvalues j = 1/`rc' {;
		matrix `tmp' = `D'[....,`j'];
		...
		if missing(`"`coleq'"') local coleq : coleq `tmp';
		 ...
		 if `j'>1 replace     d_`=`j'-1' = cond(d_`j' == d_`=`j'-1', ".x", `"`coleq1'"') in 2;
	    local coleq1 `"`coleq'"';
		local cn : word `j' of `cnames';
		if missing("`cn'") local cn : colnames `tmp';
		 replace d_`j' = "`cn'" in 3;
		forvalues i = 1/`rr'{;
  			replace d_`j' = cond(`D'[`i',`j']==.z,"",string(`D'[`i',`j'],"%25.0g")) in `=`i'+3';
			replace f_`j' = "`= `D_STARS'[`i', `j']'_" + upper(substr(trim(f_`j'),3,5)) in `=`i'+3';
		}; // forvalues i
		...
		replace d_`j' =""  			if(d_`j'==".z");
		replace d_`j' ="(dropped)"  if(d_`j'==".d");
	}; // forvalues j
	...
	foreach f in `flist' {;
	    ...
	    if "`bline'"=="1" {;
         ...
         /*
		    ...
		    local BR "<Border ss:Position='Bottom' ss:LineStyle='Continuous' ss:Weight='1' />" 
			         "<Border ss:Position='Top'    ss:LineStyle='Double'     ss:Weight='1' /> ";
		};
		*/
	    else if "`bline'"=="14" {;
		    local BR "<Border ss:Position='Bottom' ss:LineStyle='Continuous' ss:Weight='1'  />" 
		...
		if (`nstars' == 9) local fmt "(#,##`digt');(-#,##`digt')";
		else {;
			if (`nstars'>0 ) {;
				local char : word `nstars' of `starchars';
			};
			else local char;
		    local fmt "#,##`digt'&quot;`char'&quot;;-#,##`digt'&quot;`char'&quot;";
		};
	    local styles "`styles'<Style ss:ID='s`fno'_`f''>";
	    local styles "`styles'<Alignment `V' `H' ss:WrapText='1'/>";
...
cap program drop  opts_Exclusive;
...
program define getstarchars;
	...
	if `n'<=3 {; 					// Simple syntax
		...
		tokenize `stars';
		if missing("`6'") local 6 = .;
		if (`4' > `2') {;
			if (`6' > `4') local starchars "`1' `3' `5'";
			else if (`6' > `2') local starchars "`1' `5' `3'";
				 else local starchars "`5' `1' `3'";
		};
		else {;
			if (`6' > `2') local starchars "`3' `1' `5'";
			else if (`6' > `4') local starchars "`3' `5' `1'";
				else local starchars "`5' `3' `1'";
		};
		local stars "`2' `4'";
		if (`6' < .) local stars "`stars' `6'";
	};
    capture numlist "`stars'", range(>=0 <=1) sort;
	if _rc {;
		display as error "stars() -- invalid numlist";
		exit _rc;
	};
	c_local starchars `"`starchars'"';
	c_local stars "`r(numlist)'";
end; // program gettstarchars

cap program drop  Mat_Capp;
