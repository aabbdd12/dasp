...
#delim ;
set more off;
capture program drop gicur;
program define gicur, rclass;
version 9.2;
...
conf(`conf') level(`level') dec(`dec') crv(2) nona(`nona') smooth(`smooth') band(`band');
...
if (`order'==1) {;
                      ...
                      if (`crv'==2) local estima = "( Q_2(p) - Q_1(p) ) / Q_1(p) ";
...
local tit  = "Growth incidence curve"; // 
...
xlabel(`min'(`m5')`max')
plotregion(margin(zero))
legend(order(1  "Estimated difference" 2 "Growth in average"))
