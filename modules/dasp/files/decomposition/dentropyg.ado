...
qui dientropy `muk' `1' , hsize1(`_ths') hsize2(`_ths') theta(`theta') ;
cap drop matrix tmp;
matrix tmp = e(di);
local stc_within = el(tmp,1,2);
cap matrix drop tmp;

qui replace `EST3'      = `stc_within' in `kk2';
...
.`table'.row `Variable'[`kk2'] "---"  "---"  "---"  `EST3'[`kk2'] "---";
};
