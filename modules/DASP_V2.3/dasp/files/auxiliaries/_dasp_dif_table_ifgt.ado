...
cap program drop _dasp_dif_table_ifgt;
program define _dasp_dif_table_ifgt, rclass;
version 9.2;
syntax varlist(min=2 max=2)[,  
NAME1(string) NAME2(string)
M1(real 1)  M2(real 1)
S1(real 1)  S2(real 1)
PL1(real 10000)  PL2(real 10000)
DF1(int 1)   DF2(int 1)
...
local cilc = 59;
		noi di in smcl in gr "{hline 9}{c TT}{hline 80}";

                 noi di in smcl in gr "   Index {c     |}" /*
                 */ _col(14) "Estimate" _col(25) /*
                 */ "Std. Err." _col(41) "t" _col(47) /*
         */ "P>|t|" _col(`cilc') `"[`=strsubdp("`level'")'% Conf. interval]"' _col(72) "  Pov. line";
                 noi di in smcl in gr "{hline 9}{c +}{hline 80}"; 
...
if ("`conf'"=="ub") 
        ...
        di in smcl in gr "{hline 9}{c +}{hline 80}";
...
di in smcl in gr "{hline 9}{c BT}{hline 80}";
...
/* Display Ho. */
        ...
        di as txt  as txt _col(6) "degrees of freedom = " as res %8.0f `df' ;
...
/* Display Ha. */
        ...
        */             " Ha: est.(diff)  > `test'";
	    ...
	    _ttest center2 " Hb: est.(diff) >= `test'"  /*
        */             " Hb: est.(diff) != `test'" /*
        */             " Hb: est.(diff) <= `test'";
 ...
 di in smcl in gr "{hline 9}{hline 82}";
 };
