*! version 4.0.4 10nov2010 E. Leuven, B. Sianesi
cap program drop psmatch2
program define psmatch2, sortpreserve
	...
	if ("`method'"=="llr" & "`kerneltype'"=="") local kerneltype "tricube"
	...
	if ("`varlist'"!="") {
		...
		`quietly' `logit' `treat' `varlist' if `touse' [pw=`pw'], nolog
		tempvar pscore
	...
	// generate output
	if ("`outcome'"!="") _mktab `outcome', `ate' `spline' `llr' k(`kerneltype') ai(`ai') n(`neighbor') `population' pw(`pw')
...
// FORMAT OUTPUT TABLE
cap program drop _mktab
program define _mktab, rclass
syntax varlist [, ate spline llr Kerneltype(string) ai(integer 0) Neighbor(integer 1) population PW(varname)]
...
qui foreach v of varlist `varlist' {
	...
	sum `v' if _treated==1 [aw=`pw'], mean
	scalar `u1u' = r(mean)
	sum `v' if _treated==0  [aw=`pw'], mean
	scalar `u0u' = r(mean)

	sum `v' if _treated==1 & _support==1 [aw=`pw'] , mean
	scalar `m1t' = r(mean)
	local N1 = r(N)
	sum _`v' if _treated==1 & _support==1 [aw=`pw'], mean
	scalar `m0t' = r(mean)
	scalar `att' = `m1t' - `m0t'

	if ("`ate'"!="") {
		sum _`v' if _treated==0 & _support==1 [aw=`pw'], mean
		scalar `m1u' = r(mean)
		sum `v' if _treated==0 & _support==1 [aw=`pw'], mean
		scalar `m0u' = r(mean)
		local N0 = r(N)
		scalar `atu' = `m1u' - `m0u'
	...
	if (`ai'!=0) {
		...
		sum `VhatEt' if _support==1
		scalar `seatt' = sqrt(r(sum))/`N1'
		if ("`ate'"!="") {
			sum `VhatEu' if _support==1
			scalar `seatu' = sqrt(r(sum))/`N0'
			sum `VhatE'  if _support==1
...
if (`ai'==0 & `seatt'!=.) {
	di in w "Note: S.E. does not take into account that the propensity score is estimated."
}
else {
	if ("`population'"=="") di in w "Note: Sample S.E."
	else di in w "Note: Population S.E."
}
...
program define _Match_kernel
	...
	if ("`metric'"=="mahalanobis") {
		mata : _Dif_mbase("`varlist'", "`base'", "`w'") 
	}
	...
	forvalues obs = `start'/`r(N)' {
		_Dif_`metric' `varlist' if _support==1 & _treated==(`obs'<=`N0'), obs(`obs') dif(`dif') base(`base') w(`w')
		...
		if (abs(PSCORE[i] - PSCORE[jmatch])<caliper) {
			...
			for(k = 1; k<=nmatch; k++) {
				...
				if (i0 != j0 & k<=neighbors) { // note that with ties we only keep k<=neighbors id's
					N1[i, k] = obs
					WEIGHT[obs] = WEIGHT[obs] + 1/nmatch
				}
				if (nout>0) MOUTVAR[i,.] = MOUTVAR[i,.] + OUTVAR[obs,.]:/nmatch
				}
