...
#delimit;
capture program drop mcjob51;
program define mcjob51, eclass;
version 9.2;
...
mcjobpov `tot_imp',    hs(`hsize') aehs(`aehs')  lan(`lan')   xrnames(total)  pcexp(`pcexp') alpha(0)  pline(`pline')  move(`move');
tempname mat51tot ;
matrix `mat51tot'= e(est); 
...
cap drop  __imwmc;
mcjobpov `tot_imp',    hs(`hsize') aehs(`aehs')  lan(`lan')   xrnames(`slist')  pcexp(`pcexp') alpha(0)  pline(`pline')  move(`move');
tempname mat51tot ;
matrix `mat51tot'= e(est); 
};
...
mcjobpov `flist',    hs(`hsize') aehs(`aehs')  lan(`lan')   xrnames(`slist')  pcexp(`pcexp') alpha(0)  pline(`pline')  move(`move');
cap drop `drlist';
tempname mat51 ;
matrix `mat51'= e(est);

if (`wappr' != 1 ) {;
local rowsize = rowsof(`mat51');
local colsize = colsof(`mat51');
forvalues i=1/`rowsize' {;
 matrix `mat51'[ `i',`colsize'] = el(`mat51tot',`i',1);
};
};
