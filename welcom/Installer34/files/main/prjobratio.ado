...
capture program drop prjobratio;
program define prjobratio, eclass;
version 9.2;
...
qui gen `asliving' = `sliving';
if ("`aehs'" ~= "" ) qui replace `asliving' = `sliving'*`_ths'/`aehs' ;
basicratio `asliving' `osliving',  hsize(`_ths')   p1(0.1) p2(0.9);
...
if ("`lan'"~="fr")  local cnam `"`cnam' "Variation in the ratio index""';
