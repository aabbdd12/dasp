...
syntax varlist(min=1 max=1)[, HSize(varname) HGroup(varname) THETA(real 0) 
 STD(string) dec(int 6) LEVEL(real 95) DSTD(int 1)
...
local ci=100-`level';
tempvar Variable EST EST1 EST2 EST22 EST3 EST4 STD STD1 STD2 STD22 STD3 STD4;
qui gen `Variable'="";
qui gen `EST'=0;
qui gen `STD'=0;
...
qui replace `EST4'      = .              in `kk4';
	...
	forvalues i=1/`k2'{;
		  ...
		  if (`dstd'==1 | (`i'/2!=round(`i'/2))) .`table'.row `Variable'[`i'] `EST'[`i']  `EST2'[`i'] `EST22'[`i'] `EST3'[`i'] `EST4'[`i'];	        
              };
...
if ("`xlan'"=="fr")  local xtit = "Tableau ##: Décomposition de l'indice d'entropie selon...";
