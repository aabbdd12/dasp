...
program def st1reg;
syntax namelist , cluster(varname) lnexp(varname) lhhs(varname) hweight(varname) indepvars(string) csb(string);
local ig=1;
tokenize `namelist' ; 
...
gen      _s = (w`1'!=0);
capture {;
qui probit _s  `lnexp' `lhhs' `indepvars' [pw=`hweight'];
cap drop _xb;
qui predict _xb, xb;
cap drop `lambda' ;
tempvar lambda;
...
syntax namelist(min=1)[, hweight(varname) hsize(varname) expend(varname) cluster(varname) indepvars(string) 
region(varname) subround(varname) quard4(varname)  csb(string) boot(int 0)];
preserve;


set more off;
local mylist `namelist' ;
...
st1reg $goods , lnexp(`lnexp') lhhs(`lhhs') indepvars(`indepvars') cluster(`cluster') hweight(`hweight') csb(`csb') ;
/*
matrix list sig;
matrix list ome;
matrix list lam;
...
matlist AV_SHA[., 1..$indican] , border(all)  format(%6.3f)  twidth(8) left(4) title( " - Average expenditures shares (in %)");


matlist EXP_SY_E[., 1..$indican] , border(all)  format(%6.3f)  twidth(8) left(4)  title( " - Expenditure elasticities ");
matlist QUAL_SY_E[., 1..$indican] , border(all)  format(%6.3f)  twidth(8) left(4) title( " - Quality elasticities     ");
		 
matlist B_NS_ep[1..$indican, 1..$indican], border(all)  format(%6.3f)  twidth(8) left(4) title( " - Price elasticities: without quality correction | without symmetry restricted estimators");

matlist B_SY_ep[1..$indican, 1..$indican], border(all)  format(%6.3f)  twidth(8) left(4) title( " - Price elasticities: without quality correction | with symmetry restricted estimators");

matlist C_NS_ep[1..$indican, 1..$indican], border(all)  format(%6.3f)  twidth(8) left(4) title( " - Price elasticities: with quality correction | without symmetry restricted estimators");
matlist C_SY_ep[1..$indican, 1..$indican], border(all)  format(%6.3f)  twidth(8) left(4) title( " - Price elasticities: with quality correction | with symmetry restricted estimators");

if (`boot'>0){;
matrix colnames  MSTE = `mynames' ;
matrix rownames  MSTE = `mynames' ;
matlist MSTE[1..$indican, 1..$indican], border(all)  format(%6.4f)  twidth(8) left(4) title( " - Standard errors: with the bootstrap method");
};
