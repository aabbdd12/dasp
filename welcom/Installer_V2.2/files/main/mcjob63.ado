...
#delimit;
capture program drop mcjob63;
program define mcjob63, eclass;
version 9.2;
syntax varlist(min=1)[, HSize(varname) HGroup(varname) LAN(string) XRNAMES(string)  AGGRegate(string) PCEXP(varname)  WAPPR(int 1) MODEL(int 1) GVIMP(int 0) SUBS(real 0.4) MOVE(int 1) THETA(real 0) STEP(int 1)];
...
mcjobentropy `tot_imp',   hs(`hsize') aehs(`aehs')  lan(`lan')   xrnames(`slist')  pcexp(`pcexp')    move(`move')  theta(`theta') ;
tempname mat63tot ;
matrix `mat63tot'= e(est); 
};
...
mcjobentropy `flist',   hs(`hsize') aehs(`aehs')  lan(`lan')   xrnames(`slist')  pcexp(`pcexp')    move(`move')  theta(`theta') ;
cap drop `drlist';
tempname mat63 ;
matrix `mat63'= e(est);

if (`wappr'!=1) {;
local rowsize = rowsof(`mat63');
local colsize = colsof(`mat63');
forvalues i=1/`rowsize' {;
 matrix `mat63'[ `i',`colsize'] = el(`mat63tot',`i',1);
};
