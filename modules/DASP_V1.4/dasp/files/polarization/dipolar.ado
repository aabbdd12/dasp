...
#delim ;
cap program drop dipolar2;  
program define dipolar2, rclass sortpreserve;     
version 9.2;         
...
local polar=`r(mean)'/(2.0*`mu'^(1.0-`alpha'));
local tempo = r(mean);
...
capture program drop dipolar;
program define dipolar, eclass;
version 9.2;
...
LEVEL(real 95) CONF(string) DEC(int 6)];
...
dipolar2 `1' , hweight(`hweight1') hsize(`ths1') alpha(`alpha') fast(`fast') conf(`conf') level(`level') vab(`vab') dname("Distribution 1");
...
dipolar2 `2' , hweight(`hweight2') hsize(`ths2') alpha(`alpha') fast(`fast') conf(`conf') level(`level') vab(`vab') dname("Distribution 2");
if (`vab'==1) {;
tempvar vd ve vf;
qui gen `vd'=__va;
qui gen `ve'=__vb;
...
local index = "Duclos, Esteban and Ray index of polarisation (2004) (DER)";
local tyind1="DER";
local tyind2="DER";
if ("`type'"=="not") local not = "Not normalized ";
