...
capture program drop mbasicpov;
program define mbasicpov, eclass;
syntax varlist(min=1) [, HSize(varname) HGROUP(varname) AEHS(varname)  PLINE(varname)  ALpha(real 0) XRNAMES(string) TOT(int 0) MTRANS(int 1) ];
preserve; 
tokenize `varlist';
_nargs   `varlist';
...
capture program drop genjobpov;
program define genjobpov, eclass;
version 15;
...
//set trace on ;
set tracedepth 2;
mbasicpov `varlist' ,  hsize(`_ths')   hgroup(`hgroup_`i'') alpha(`alpha') pline(`pline')  xrnames(`xrnames') tot(`tot') mtrans(0);
...
mbasicpov `varlist' ,  hsize(`_ths')    alpha(`alpha') pline(`pline')  xrnames(`xrnames') tot(`tot') mtrans(0) ;
#delimit cr
