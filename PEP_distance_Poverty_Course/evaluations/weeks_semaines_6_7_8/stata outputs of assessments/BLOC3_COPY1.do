...
Also, it is often difficult to identify a common set of goods and services (key for estimating an absolute poverty line) 
that can are consumed by a broad range of the population in those countries.
*/
...
//  Q3:
gen     ae_exp2=ae_exp
replace ae_exp2=ae_exp2*(1+0.10) if region==3
...
/*Although the average growth among the poor is negative (as estimated by the Ravallion and Chen Index), 
we observe a relative pro-poor growth (since the overall average growth is lower than that just for the poor)*/

//  Q6:
dfgtgr inc_t1 inc_t2, alpha(1) pline(10.2)
/*the increase in the poverty gap is driven by the growth component (as see earlier, most of percentiles
experience a decrease in their income). The redistribution component only slightly buffers such a poverty increase.*/
