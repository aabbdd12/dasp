...
forvalues s=1/`simula' {;
if (`gvimp' == 1)   mcjob41 `vlist',   hs(`hsize') hgroup(`hgroup') lan(en)  xrnames(`slist')  aggr(`taggregate') pcexp(`1')   wappr(`meas') model(`model') subs(`subs')  gvimp(`gvimp') move(`move') step(`s');
};
if (`move' == -1) {;
local mcolname Concentrated ;
if `mpart' >= 1 {;
forvalues i=1/`mpart' {;
local mcolname `mcolname' Scenario_`i' ;
}; 
};
...
distable `mat110', dec(2) atit(`langr') head1(11_`i': `mytit')  head2(`head2') ;
mk_xtab_mc `1' ,  matn(`mat110') dec(2) xfil(`xfil') xshe(table_11_`i') xtit(11_`i': `mytit') xlan(`lan') dste(0);
