...
program define imdpov_af, rclass;
version 9.2;
syntax varlist [,  HSize(string) HGroup(varname) GNumber(int -1)
nembarg(int 1) 
dec(int 3)
...
tempname table;
	...
	forvalues i=1/`k2'{;
		  ...
		  if (`dstd'==1 | (`i'/2!=round(`i'/2))) .`table'.row `Variable'[`i'] `EST1'[`i']  `EST2'[`i'] `EST3'[`i'] `EST4'[`i'] `EST5'[`i'];
...
tempname table;
	...
	forvalues i=1/`k2'{;
		  ...
		  if (`dstd'==1 | (`i'/2!=round(`i'/2))) .`table'.row `Variable'[`i']   `CEST2'[`i'] `CEST3'[`i'] `CEST4'[`i'] `CEST5'[`i'];
