...
#delimit;
capture program drop mcjobgr5;
program define mcjobgr5, eclass;
version 9.2;
syntax varlist(min=1)[, HSize(varname) HGroup(varname) LAN(string) XRNAMES(string) AGGRegate(string) PCEXP(varname) PLINE(varname) MIN(real 0) MAX(real 1000) OGR(string) 
