...
capture program drop genjobatk;
program define genjobatk, eclass;
version 15;
...
mbasicatk `1' ,  hsize(`_ths')  hgroup(`hgroup_`i'')   epsilon(`epsilon') 
...
tokenize  `varlist'
if "`hgroup'" ~= "" {
mbasicatk  `1',  hsize(`_ths')  epsilon(`epsilon') 
#delimit cr
...
if "`hgroup'" == "" {
mbasicatk  `varlist',  hsize(`_ths')  epsilon(`epsilon') 
#delimit cr
matrix _ms_pop = e(mmss)'
...
local tmatna `tmatna' _ms_pop
	   ...
	   local index = "Atkinson";
   	  di _n as text in white "{col 5}Index            :  `index' index"
                            di as text     "{col 5}Epsilon          :  `epsilon'"
...
if ("`xfil'" ~= "" ) {
                       local gro = 0	
if ("`hgroup'" ~= "" ) local gro = 1
if "`xtit'"==""   local xtit Table ##: Atkinson index
if `gro' == 1     local xtit `xtit' by population groups
local note1 Parameter epsilon = `epsilon'
#delimit ;
