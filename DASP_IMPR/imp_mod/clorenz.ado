...
version 9.2;
args www yyy rank type min max perca gr ng;
quietly {;
local perca1=`perca'+1;
if (_N<`perca1') qui set obs `perca1';
preserve;
...
local _finlp_`av'=`lpi';
};
/*
qui keep in 1/`perca1';
set matsize `perca1';
cap matrix drop _xx;
...
cap drop _finlp;
qui gen _finlp=0;
forvalues j=1/`perca1' {;
qui replace _finlp = `_finlp_`j'' in `j';
};
};
end;
...
syntax varlist(min=1)[, HWeight(varname) HSize(varname) HGroup(varname)
 ...
 LRES(int 0)  SRES(string) DGRA(int 1) SGRA(string) EGRA(string) POP(string) PERCentiles(int 100) *];
...
_get_gropts , graphopts(`options') ;
	...
	local perca=`percentiles';
local perca1=`percentiles'+1;
...
if ("`min'"=="")  local min  = 0;
if ("`max'"=="")  local max  = 1;
lorenz2 `fw' `1' `rank' `type' `min' `max' `perca' ;
rename _finlp  _corypop;
if ("`dif'"~="c1" & "`pop'" != "no") {;
...
lorenz2 `fw' ``k'' `rank' `type' `min' `max' `perca';
...
lorenz2 `fw' `1' `rank' `type' `min' `max' `perca' `hgroup' `k' ;
};
rename _finlp _cory`k';
};
local m5=(`max'-`min')/5;
local step=(`max'-`min')/`perca';
