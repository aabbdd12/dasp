...
#delim ;
set more off;
capture program drop mcprc;
program define mcprc , eclass sortpreserve;
version 9.2;
...
tempname price`it' elas`it' size`it' nfirm`it'  step`it';
qui gen `price`it'' = . ;
qui gen `elas`it''  = . ;
qui gen `size`it''  = . ;
qui gen `nfirm`it'' = . ;
...
qui replace ___step`it' = "Concentrated Market    "  in `posa';
qui replace `nfirm`it'' = `nfirms' in `posa';
local pos = 2;
local mpart1 = `mpart'+1;
local N = `nfirms';
forvalues i=2/`mpart1' {;
...
qui replace ___step`it' = "Step : `j'"   in `posa';
...
qui replace ___step`it' = "Competitive  Market "    in `posa';
...
tempname table;
      ...
      .`table'.sep, mid;
	  forvalues i=1/`pos' {;
      if `st`it'' != 2 .`table'.row ___step`it'[`i']    `size`it''[`i']    `elas`it''[`i']  `price`it''[`i']; 
	  if `st`it'' == 2 .`table'.row ___step`it'[`i']    `nfirm`it''[`i']   `elas`it''[`i']  `price`it''[`i']; 
