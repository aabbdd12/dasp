...
if (`gmodifier' == 0) {;
mk_xtab_tr `1' ,  matn(rMSTE) dec(`dec') xfil(`xfil') xshe(Table_08) xtit("Table 08: Standard errors: with the bootstrap method ") xlan(en) dste(0) ;
matlist MSTE[1..$indican, 1..$indican], border(all)   format(%`cwidth'.`dec'f)  twidth(`cwidth') left(2) title( "Table 08: Standard errors: with the bootstrap method");
};
};
...
mk_xtab_tr `1' ,  matn(DEC_ELAS) dec(`dec') xfil(`xfil') xshe(Table_09) xtit("Table 09: Own Price Elasiticies by hgroups ") xlan(en) dste(0) ;
...
forvalues j=1/10 {;
preserve;
qui keep if hgroup == `j' ;
qui duvm cereal bread flour  semolina couscous pasta, hhsize(hhsize) expend(hhexp) hweight(sweight) cluster(cluster) region(region) indcat(sex marital education activity) indcon(age) ;
matlist C_NS_ep;
forvalues i=1/6 {;
local elas_gr_`i'_`j'  = el(C_NS_ep,`i',`i') in `j' ;
};
restore;
};
...
forvalues j=1/10 {;
forvalues i=1/6 {;
qui replace elas_gr_`i' = `elas_gr_`i'_`j'' in `j' ;
};
};
