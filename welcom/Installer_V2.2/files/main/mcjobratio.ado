...
program define _dinineq2, rclass ;    
version 9.2;         
...
program define basicratio, rclass;
version 9.2;
...
capture program drop mcjobratio;
program define mcjobratio, eclass;
version 9.2;
...
basicratio `asliving' `pcexp',  hsize(`_ths')  p1(0.1) p2(0.9);
};
if (`move'==-1)     {; 
tempvar sliving asliving osliving;
qui gen `sliving' = `pcexp'+ `total';
...
if ("`aehs'" ~= "" ) qui replace `asliving' = `sliving'*`_ths'/`aehs' ;
gen `osliving' = `asliving' ;
basicratio `pcexp' `asliving',  hsize(`_ths')  p1(0.1) p2(0.9);
};
...
/*
sum  `_ths'  tot_integ;
iratio `pcexp' `asliving' pcexpf,  hsize(`_ths');
*/
qui _nargs    `varlist';
...
qui gen `asliving' = `sliving';
if ("`aehs'" ~= "" ) qui replace `asliving' = `sliving'*`_ths'/`aehs' ;
if (`move'== 1)   basicratio `asliving' `osliving',  hsize(`_ths')   p1(0.1) p2(0.9);
if (`move'==-1)   basicratio `asliving' `pcexp' ,    hsize(`_ths')   p1(0.1) p2(0.9);
...
qui gen `asliving' = `sliving';
if ("`aehs'" ~= "" ) qui replace `asliving' = `sliving'*`_ths'/`aehs' ;
basicratio `pcexp' `osliving',  hsize(`_ths')   p1(0.1) p2(0.9);
};
