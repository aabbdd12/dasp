...
help for {hi:cprog }{right:Dialog box:  {bf:{dialog cprog}}}
{hline}

{title:Lorenz and concentration curves}

{p 8 12}{cmd:cprog}  {it:varlist}{cmd:,} [ 
...
{p}{cmd:cprog} produces progressivity curves (PR(p)) for a given list of variables (components).{p_end}
{p}Let X be gross income.{p_end}
{p 4 8}{inp:. A tax T is Tax Redistribution (TR) progressive if: }{p_end}
{p 4 8}{inp:. PR(p) = L_X(p) - C_T(p) > 0 for all p in ]0, 1[ }{p_end}

{p 4 8}{inp:. A transfer B is Tax Redistribution (TR) progressive if: }{p_end}
{p 4 8}{inp:. PR(p) = C_B(p) - L_X(p) > 0 for all p in ]0, 1[ }{p_end}

{p 4 8}{inp:. A tax T is Income Redistribution (IR) progressive if:}{p_end}
{p 4 8}{inp:. PR(p) = C_X-T(p) - L_X(p)  > 0 for all p in ]0, 1[ }{p_end}

{p 4 8}{inp:. A transfer B is Income Redistribution (IR) progressive}{p_end}
{p 4 8}{inp:. PR(p) = C_X+B(p) - L_X(p)  > 0 for all p in ]0, 1[ }{p_end}
...
{p 0 4} {cmdab:rank}    Required to set the ranking variable, which is usually gross income, denoted by X above. {p_end}
...
{p 0 4} {cmdab:appr}     By default is TR. Select option "IR" for the Income Redistribution approach. {p_end}
