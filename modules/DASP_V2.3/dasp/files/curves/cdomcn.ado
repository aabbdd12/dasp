...
capture program drop cdomcnc2;
program define cdomcnc2, rclass;
version 9.2;
...
if (`alpha' ~= 0 & `pline' ~=0 & "`fnor'" == "yes")           qui replace    `num' = `hs'*((`pline'-`1')/`pline')^`alpha'  if (`pline'>`1');
if (`alpha' ~= 0 & `pline' ~=0 & "`fnor'" ~= "yes")           qui replace    `num' = `hs'*((`pline'-`1'))^`alpha'          if (`pline'>`1');

if ("`cnor'" ~= "yes") {;

if (`alpha' > 0) {;
if (`al1' == 0 & `pline' ~=0 & "`fnor'" == "yes")               qui replace    `numi' = `hs'/`pline'*(`pline'> `1')*`component'  ;
if (`al1' == 0 & `pline' ~=0 & "`fnor'" ~= "yes")               qui replace    `numi' = `alpha'*`hs'*(`pline'> `1') if (`pline'>`1')*`component' ;
if (`al1' ~= 0 & `pline' ~=0 & "`fnor'" == "yes")               qui replace    `numi' = `alpha'/`pline'*`hs'*((`pline'-`1')/`pline')^`al1'*`component'  if (`pline'>`1');
if (`al1' ~= 0 & `pline' ~=0 & "`fnor'" ~= "yes" )              qui replace    `numi' = `alpha'*`hs'*((`pline'-`1'))^`al1'*`component'  if (`pline'>`1');
...
if ("`cnor'" == "yes") {;
if (`alpha' > 0) {;
if (`al1' == 0 & `pline' ~=0 & "`fnor'" == "yes")            qui replace    `numi' = `alpha'/`pline'*`hs'*(`pline'> `1')*`component' ;
if (`al1' == 0 & `pline' ~=0 & "`fnor'" ~= "yes")            qui replace    `numi' = `alpha'*`hs'*(`pline'> `1')*`component' ;
if (`al1' ~= 0 & `pline' ~=0 & "`fnor'" == "yes")            qui replace    `numi' = `alpha'/`pline'*`hs'*((`pline'-`1')/`pline')^`al1'*`component'   if (`pline'>`1');
if (`al1' ~= 0 & `pline' ~=0 & "`fnor'" ~= "yes")            qui replace    `numi' = `alpha'*`hs'*((`pline'-`1'))^`al1'*`component'   if (`pline'>`1');
...
capture program drop gcdomcnc2;
program define gcdomcnc2, rclass;
version 9.2;
...
if ("`dci'"=="yes") mkmat  `vimpp' `_limpp' `_uimpp', matrix (_xx);
restore;
end;
...
capture program drop cdomcn;
program define cdomcn, rclass;
version 9.2;
...
DEC(int 9)
DSTE(int 1)
LRES(int 0) SRES(string) DCI(string) LEVEL(real 95) STITLE(string)
...
cdomcnc2 `inc', hweight(`hweight')  hsize(`hsize')  hgroup(`hgroup') ngroup(`kk') component(`com1') prc(`prc')  
...
return scalar cdomcn = `r(impp)' ; 
return scalar scdomcn = `r(simpp)' ; 
...
if ("`dci'"=="yes" & "`conf'"=="ts") local mcmd  = "`mcmd'" + "(rarea  _coryl`k'  _coryu`k' _corx, fcolor(blue) fintensity(20) lcolor(white) lwidth(thin)  sort ) ";
local f=`k';
...
gcdomcnc2 `inc', hweight(`hweight') hgroup(`hgroup') ngroup(`kk') prc(`prc') 
...
gcdomcnc2 `inc', hweight(`hweight') component(``k'') prc(`prc') 
...
if ("`conf'"=="ts") {;
 rename _xx1 _cory`k';
 rename _xx2 _coryl`k';
 rename _xx3 _coryu`k';
