...
{* June 2018}{...}
{hline}
{hi:TOBATREF : Tobacco Tax Reforms}
help for {hi:tobatref }{right:Dialog box:  {bf:{dialog tobatref}}}
{hline}
{title:Syntax} 
{p 8 10}{cmd:tobatref}  {it:varlist (2 varnames)}  {cmd:,} [ 
...
{title:Description}
{p} The {cmd:tobatref} module is conceived to automate the estimation the results of the tobacco tax reforms on social welfare based on the methods of Fuchs & Meneses  (2017).
...
by default. {p_end}
{title:The tobatref results}  
{p} The following lists shows the produced tables and graphs:{p_end}
...
{p 4 8}{inp:[04] Table 31: Price increase (past estimate in %)}{p_end}
...
{p 0 6} {cmd:inisave:}    To save the TTRWEL project information. Mainly, all inserted information in the dialogue box will be save in this file. In another session, the user can open the project using the command tobatrefl_ini_trw followed by the name of project. This command will initialize all of the information of the aTTRWEL dialog box. {p_end}
...
{it:{help by_option}}).  {p_end}
{p 0 6} {cmd:felas:}    To indicate  the path and the name of the Stata data file that contains the elasticities by deciles. {p_end}
{p 0 6} {cmd:elasm:}    To indicate name of the variable of the medium elasticity. {p_end}
{p 0 6} {cmd:elasl:}    To indicate name of the variable of the lower elasticity. {p_end}
{p 0 6} {cmd:elasu:}    To indicate name of the variable of the upper elasticity. {p_end}
{p 0 6} {cmd:mexexp:}   To indicate the total population medical expenditures on the tobacco illnesses. {p_end}
{p 0 6} {cmd:tyll:}     To indicate the total years of life lost due to tobacco consumption at population level. {p_end}
{p 0 6} {cmd:princ:}    To indicate the price increase of the simulated tobacco tax reform. {p_end}
{p 0 6} {cmd:dec:}    To indicate number of decimals of the displayed results. {p_end}
{p 0 6} {cmd:ste:}    Add the option ste(0) to do not display the tables with the standard errors. {p_end}

{title:Example(s):}
tobatref rhexp rs_exp_cigarettes, hsize(member)  decile(decile) ///
