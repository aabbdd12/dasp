...
version 9.2;
args www yyy pline min max gr ng;
preserve;
...
qui replace _finqp=max(0,`pline'-`lqi')  in `av';
};
...
cap drop _wy;
cap drop _lp;
qui replace `yyy'=max(0,`pline'-`yyy');
gen _ww = sum(`www');
gen _wy = sum(`www'*`yyy');
local suma = _wy[_N];
cap drop _pc;
...
local tit2="PG";
if ("`type'"=="cpg") local tit2="CPG";
local tit1="Poverty Gap";
if ("`type'"=="cpg") local tit1="Cumulative poverty gap";
...
if ("`type'"=="pg")  qui cpov1 `fw' ``k'' `pline'  `min' `max' ;
if ("`type'"=="cpg") qui cpov2 `fw' ``k'' `pline'  `min' `max' ;
...
if ("`type'"=="pg")  qui cpov1 `fw' `1' `pline' `min' `max' `hgroup' `k';
if ("`type'"=="cpg") qui cpov2 `fw' `1' `pline' `min' `max' `hgroup' `k';
