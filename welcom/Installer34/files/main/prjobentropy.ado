...
capture program drop prjobentropy;
program define prjobentropy, eclass;
version 9.2;
...
qui gen `asliving' = `sliving';
if ("`aehs'" ~= "" ) qui replace `asliving' = `sliving'*`_ths'/`aehs' ;
basicentropy `asliving' `osliving',  hsize(`_ths')  theta(`theta');
...
if ("`lan'"~="fr")  local cnam `"`cnam' "Variation in the entropy index""';
