...
version 9.2;
syntax varlist(min=1)[,  HWEIGHT(string) HSize(string) HGroup(varname) NGroup(int 1) prc(real 1) component1(string) component2(string)  ALpha(real 1) PLINE(real 60000)
...
if ("`est'"=="rat") {;
qui nlcom (_b[`numi2'] / _b[`numi1']), iterate(50000) ;
};
...
if (`j'!=601 & `j'/6==round(`j'/6))  dis "." ,  _continue;
if (`j'/60==round(`j'/60)) dis " "  %4.2f `j'/6 " %";
...
HSize(varname)  HGroup(varname)  inc(varname)  ORDER(real 1) PLINE(real 60000)
PRC(real 1)
cnor(string) est(string)
DEC(int 9)
DSTE(int 1)
