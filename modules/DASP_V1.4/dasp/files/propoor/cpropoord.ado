...
cap drop  _lb;    
cap drop  _ub;   
mat colnames RES	= _perc _dif _ub _lb; 
svmat RES, name(col) ;
...
qui keep in 1/101;
                   ...
                   local mylist = "_perc _dif _lb _ub";
if("`conf'"=="lb") local mylist = "_perc _dif _lb";
if("`conf'"=="ub") local mylist = "_perc _dif _ub"; 
...
local drhline = "";
if (`dhol'==1) local drhline ="(line   _nl _perc,  sort clcolor(gs9) lpattern(solid) `hor_line_opts' 
)";
quietly {;
...
if("`conf'"=="ts") {;
twoway 
(rarea _lb _ub _perc, sort blcolor(ltbluishgray) 
blwidth(none)
blcolor(gs14) 
bfcolor(gs14) 
