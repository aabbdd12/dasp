...
cap program drop _dis_dasp_table;
program define _dis_dasp_table, rclass;
version 9.2;
...
tokenize `namelist' ;
         ...
         */ "P>|t|" _col(`bal5') `"[`=strsubdp("`level'")'% Conf. Interval]"' ;
...
#delimit ;
tempname aa bb cc;
matrix `aa' = (101222136.03330, 5.002 \ 
 .7777, 1 4 \ 
,.77770, 14537646.4;
matrix `bb' = (4.3333 , 3  \ 4.7777, 3);
matrix `cc' = (10.3333 , 1.5 );
#delimit ;
sysuse bkf98I, replace ;
_dis_dasp_table_`aa'  `bb'`cc',  df11000)   level(95) conf(ub) hgroup(gse) zone dec1(3) dec2(2) pop(1)   ;
_dis_dasp_table `cc',  df11000)   level(95) conf(ub) hgroup(gse zone) dec1(3) dec2(2) pop(1) gro(0)  ;
_dis_dasp_table `aa' `bb' ,  df11000)   level(90) conf(ub) hgroup(gse zone) dec1(3) dec2(2) pop(0)  ;
