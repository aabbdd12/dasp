...
local ldif = round(`est',0.000000000000001); 

qui replace _zz= "`ldif'" in `pos';

local llb = round(`llb',0.000000000000001); 
qui replace _lb= "`llb'" in `pos';

local lub = round(`lub',0.000000000000001); 
...
MIN1(real 0) MAX1(real 10000) PAR1(int 20)
MIN2(real 0) MAX2(real 10000) PAR2(int 20) 
LEVEL(real 95) CONF(string) PROCESS(int 1)];
...
min2(`min2') max2(`max2') par2(`par2')
al1(`alpha1') al2(`alpha2') 
level(`level') conf(`conf') process(1);
...
min2(`min2') max2(`max2') par2(`par2')
al1(`alpha1') al2(`alpha2') 
level(`level') conf(`conf') process(2);
...
if ("`conf'"=="lb") local mylist = "__xx __yy __zz __st  __lb";
if ("`conf'"=="ub") local mylist = "__xx __yy __zz __st  __ub";
...
/*************************/
   ...
   file write myfile `"set out "`sgemf'"' _n;
   if ("`conf'"=="lb")  file  write myfile  `"splot "_coordinates_l.txt" using 1:2:3 with lines t "Lower-bounded" "';
   if ("`conf'"=="ts")  file  write myfile  `"splot "_coordinates_d.txt" using 1:2:3 with line t "Difference"  , "_coordinates_l.txt" using 1:2:3 with line t "Lower-bounded" , "_coordinates_u.txt" using 1:2:3 with line t "Upper-bounded" "';
   if ("`conf'"=="ub")  file  write myfile  `"splot "_coordinates_u.txt" using 1:2:3 with lines t "Upper-bounded" "';
   ...
   if ("`conf'"=="lb")  file write myfile2  `"splot "_coordinates_l.txt" using 1:2:3 with lines t "Lower-bounded" "';
   if ("`conf'"=="ts")  file write myfile2  `"splot "_coordinates_d.txt" using 1:2:3 with line t "Difference"  , "_coordinates_l.txt" using 1:2:3 with line t "Lower-bounded" , "_coordinates_u.txt" using 1:2:3 with line t "Upper-bounded" "';
   if ("`conf'"=="ub")  file write myfile2  `"splot "_coordinates_u.txt" using 1:2:3 with lines t "Upper-bounded" "';
   file close myfile2;
