...
if ("`conf'" == "lb") local plb=`slevel'/100;

if ("`conf'" == "ub") local pub=(100-`slevel')/100;
...
if ("`conf'"=="ts" | "`conf'"=="lb") local pgr `pgr' (area `f' `x' if `F'<=`plb', fcolor(blue) lcolor(blue)   ) ;
if ("`conf'"=="ts" | "`conf'"=="ub") local pgr `pgr' (area `f' `x' if `F'>=`pub', fcolor(blue) lcolor(blue)   ) ;
local pgr `pgr' , 
