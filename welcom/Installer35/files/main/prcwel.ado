...
local all_vars `varlist' `hsize' `pline' `hgroup' ;
 ...
 if ("`inisave'" ~="") {;
  ...
  if (`nitems'==0 & `ioc'==0) {;
    dis _ " ** WARNING: No infomation for the direct effect or the indirect effetcs is initialised."  ;
	exit ;
  };
	qui svyset ;
...
if `ioc' == 0 local impvlist1 `impvlist' ;
if `ioc' == 1 local impvlist1 `impvlist' __impwell_ind ;
if `ioc' == 1  local tmp2 = " |"+"Indirect Effect "; 
if `ioc' == 0  local tmp2 = " |" ; 
local impslist1 `impslist' `tmp2';
if `ioc' == 0 local tmp2 = " |"+" Total Effect"; 
if `ioc' == 1  local impvlist `impvlist' __impwell_ind __impwell_total;
if `ioc' == 0  local impvlist `impvlist' __impwell_total;
local impslist `impslist' `tmp2' ;
...
if `nitems' != 0 {;
 ...
 if `ioc'==0 qui replace __impwell_total = __impwell_total;
 if `ioc'==1 qui replace __impwell_total = __impwell_total+ __impwell_ind ;
 };
